...
Запусти `scan.py` локально, чтобы пройти авторизацию и создать `.session` файл:
...
Следуй инструкциям Pyrogram для авторизации. Будет создан файл `scanner_session.session` (или `my_mirror_bot.session` для основного бота). Положи его в папку проекта.

С помощью `scan.py` также можно узнать реальные числовые ID каналов — напиши что-нибудь в канал и прочитай вывод в консоли.
